...
export function PromptInputFullLineComponent({prompt, setPrompt}: PromptInputProps) {
  ...
  }, [prompt, selectedConversationId, setPrompt]);
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full flex-row items-center justify-between px-3 pb-3">
        ...
        <Button isIconOnly color={!prompt ? "default" : "primary"} isDisabled={!prompt} radius="full" size="sm" type="submit" variant="solid">
          <Icon className={cn("[&>path]:stroke-[2px]", !prompt ? "text-default-600" : "text-primary-foreground")} icon="solar:arrow-up-linear" width={20} />
        </Button>
      </div>
    </Form>
  );
